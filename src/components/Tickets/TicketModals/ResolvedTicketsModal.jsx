...
const ResolvedTicketsModal = ({ show, setShow, resolvedTickets = [] }) => {
  ...
  const assignees = useMemo(
    () => [
      ...new Set(
        resolvedTickets.map((t) => t.assignedTo?.name).filter(Boolean),
      ),
    ],
    [resolvedTickets],
  );
              ...
              <thead className="sticky top-0 bg-gray-50 z-10">
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-100"
                    >
                      {h}
                    </th>
                  ))}
